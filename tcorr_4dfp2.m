function tcorr_4dfp2(fMRIfilename,ROI_Filename,DFND_Filename, CorrelMatFileRoot, varargin )
	...
	%'ROISingleVol' informs the program that the ROI file has only one
	...
	%'Formatfile' and followed by a avi format file informs the program of which frames to include in the analysis. 
	%'Format' and followed by a logical array informs the program of which frames to include in the analysis. 
	...
	while ( i <= length(varargin)) 
		...
		switch varargin{i}
			...
			case 'Formatfile'
				if length(varargin) >= i + 1
					i = i + 1;
					fmt = format2logic(varargin{i});
				else
					error('missing format filename')
				end
			case 'Format'
				if length(varargin) >= i + 1
					i = i+1;
					if islogical(varargin{i})
						...
						error('Format is not a logical array')
					end
				else
	...
	if ( ~exist('fmt','var') ) 
		fmt = ones([1 nframes]) == 1;
	elif ( nframes ~= numel(fmt) ) 
		error('Format string has a different length then the numberof frames in the input image');
	end
	...
	if ( all == 1 || CCR > 0  )
		if ( nROI ~= 1)
			...
			CorrelMat(:) = 1e-37;
			CorrelMat(ROIsSize ~= 0,ROIsSize ~= 0) = correl1( ROIAvg(fmt,ROIsSize ~= 0))';
	...
	if ( all == 1 || tcorr == 1 || zfrm == 1)
		...
		CorrelImg.voxel_data(dfnd, ROIsSize ~= 0) = correl2(Image(:, fmt)',ROIAvg(fmt,ROIsSize ~= 0));
		if ( all == 1 || tcorr == 1)
